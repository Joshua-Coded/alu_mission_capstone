...
describe('ContributionService', () => {
  ...
  describe('create', () => {
    it('should create a contribution', async () => {
      ...
      expect(contributionModel.create).toHaveBeenCalled();
    });
  });
  ...
  describe('getUserContributions', () => {
    it('should return user contributions', async () => {
      ...
      mockContributionModel.countDocuments.mockReturnValue({
        exec: jest.fn().mockResolvedValue(1),
      });

      const result = await service.getMyContributions('user123', 1);
      expect(result.contributions).toEqual(mockContributions);
      expect(mockContributionModel.find).toHaveBeenCalledWith({ userId: 'user123' });
    });
  });
});
...
describe('ProjectsService', () => {
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException if project not found', async () => {
      ...
      await expect(service.findOne('invalid')).rejects.toThrow('Project not found');
    });
  });
  ...
  describe('removeFromFavorites', () => {
    it('should remove project from favorites', async () => {
      mockFavoriteModel.findOneAndDelete.mockResolvedValue({ user: 'user123', project: 'project123' });

      const result = await service.removeFromFavorites('user123', 'project123');
...
describe('UsersService', () => {
  ...
  describe('updateWalletAddress', () => {
    ...
    it('should throw error if wallet already exists', async () => {
      // Mock: wallet exists for different user
      mockUserModel.findOne.mockReturnValue({
  ...
  describe('verifyEmail', () => {
    it('should verify user email', async () => {
      const mockUnverifiedUser = {
        ...
        save: jest.fn().mockResolvedValue({ ...mockUser, emailVerified: true }),
      };
...
describe('Service Integration Tests', () => {
  it('should integrate services correctly', () => {
    // Basic integration test to ensure all services can be instantiated together
